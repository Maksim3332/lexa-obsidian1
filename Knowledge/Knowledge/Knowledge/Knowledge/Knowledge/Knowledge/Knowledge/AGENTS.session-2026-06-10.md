...
# ===== SubAgent Roles (формализованные роли) =====
# Команды: agent-role list | switch <role> | describe <role> | current
# general   — Лёха: полный доступ, партнёр по экосистеме (default)
# planner   — Plan: read-only, стратегия и планы
# builder   — Build: полный доступ, быстрая реализация
# serega    — Серёга: read-only, архитектура и ревью
AGENT_ROLE="/usr/local/bin/agent-role"
$AGENT_ROLE switch general 2>/dev/null || true
echo "  Текущая роль: $($AGENT_ROLE current 2>/dev/null | head -1)"
...
# Функция: авто-перезапуск Лёхи (OpenCode) с моделью mistral-medium-3.5 при обрыве сессии. Серёга живёт внутри OpenCode как таб @serega.
...
# Если я начинаю сообщение с ключевого слова "ПРОМТ:" — это значит, что тебе нужно преобразовать мои мысли и заметки в чёткий, структурированный промт для агента (тебя самого, Серёги, Макса или другого).
# 
# Ты должен:
